...
from feature_extraction.feature_extractors import (
    AnchorBasedCollaborativeFiltering, ConcatenateFeatureExtractors, Embedding,
    EmbeddingW, FeatureExtractor, PrototypeEmbedding)


class FeatureExtractorFactory:
    @staticmethod
    def create_models(
        ft_ext_param: dict, n_users: int, n_items: int
    ) -> Tuple[FeatureExtractor, FeatureExtractor]:
        """
        ...
        """
        assert "ft_type" in ft_ext_param, (
            "Type has not been specified for FeatureExtractor! "
        ...
        if ft_type == "detached":
            ...
            if (
                "prototypes" in ft_ext_param["user_ft_ext_param"]["ft_type"]
                and ft_ext_param["item_ft_ext_param"]["ft_type"] == "embedding"
            ):
                # User Proto
                ...
                item_feature_extractor = FeatureExtractorFactory.create_model(
                    ft_ext_param["item_ft_ext_param"], n_items, user_n_prototypes
                )

            elif (
                "prototypes" in ft_ext_param["item_ft_ext_param"]["ft_type"]
                and ft_ext_param["user_ft_ext_param"]["ft_type"] == "embedding"
            ):
                # Item Proto
                item_n_prototypes = ft_ext_param["item_ft_ext_param"]["n_prototypes"]

                user_feature_extractor = FeatureExtractorFactory.create_model(
                    ft_ext_param["user_ft_ext_param"], n_users, item_n_prototypes
                )
                item_feature_extractor = FeatureExtractorFactory.create_model(
                    ft_ext_param["item_ft_ext_param"], n_items, embedding_dim
                )
                ...
                raise ValueError(
                    "Combination of ft_type of user/item feature extractors not valid for prototypes"
                )
            ...
            item_n_prototypes = ft_ext_param["item_ft_ext_param"]["n_prototypes"]
            user_n_prototypes = ft_ext_param["user_ft_ext_param"]["n_prototypes"]
            user_use_weight_matrix = ft_ext_param["user_ft_ext_param"][
                "use_weight_matrix"
            ]
            item_use_weight_matrix = ft_ext_param["item_ft_ext_param"][
                "use_weight_matrix"
            ]

            assert (
                not user_use_weight_matrix and not item_use_weight_matrix
            ...
            user_embed.embedding_layer.weight = (
                user_proto.embedding_ext.embedding_layer.weight
            )
            item_embed.embedding_layer.weight = (
                item_proto.embedding_ext.embedding_layer.weight
            )

            user_feature_extractor = ConcatenateFeatureExtractors(
                user_proto, user_embed, invert=False
            )
            item_feature_extractor = ConcatenateFeatureExtractors(
                item_proto, item_embed, invert=True
            )
    ...
    @staticmethod
    def create_model(
        ft_ext_param: dict, n_objects: int, embedding_dim: int
    ) -> FeatureExtractor:
        """
        ...
        if ft_type == "embedding":
            max_norm = ft_ext_param["max_norm"] if "max_norm" in ft_ext_param else None
            only_positive = (
                ft_ext_param["only_positive"]
                if "only_positive" in ft_ext_param
                else False
            )
            ...
            max_norm = ft_ext_param["max_norm"] if "max_norm" in ft_ext_param else None
            out_dimension = (
                ft_ext_param["out_dimension"]
                if "out_dimension" in ft_ext_param
                else None
            )
            use_bias = ft_ext_param["use_bias"] if "use_bias" in ft_ext_param else False
            model = EmbeddingW(
                n_objects, embedding_dim, max_norm, out_dimension, use_bias
            )
        elif ft_type == "prototypes":
            n_prototypes = (
                ft_ext_param["n_prototypes"] if "n_prototypes" in ft_ext_param else None
            )
            max_norm = ft_ext_param["max_norm"] if "max_norm" in ft_ext_param else None
            sim_proto_weight = (
                ft_ext_param["sim_proto_weight"]
                if "sim_proto_weight" in ft_ext_param
                else 1.0
            )
            sim_batch_weight = (
                ft_ext_param["sim_batch_weight"]
                if "sim_batch_weight" in ft_ext_param
                else 1.0
            )
            reg_proto_type = (
                ft_ext_param["reg_proto_type"]
                if "reg_proto_type" in ft_ext_param
                else "soft"
            )
            reg_batch_type = (
                ft_ext_param["reg_batch_type"]
                if "reg_batch_type" in ft_ext_param
                else "soft"
            )
            cosine_type = (
                ft_ext_param["cosine_type"]
                if "cosine_type" in ft_ext_param
                else "shifted"
            )
            use_weight_matrix = (
                ft_ext_param["use_weight_matrix"]
                if "use_weight_matrix" in ft_ext_param
                else False
            )

            model = PrototypeEmbedding(
                n_objects,
                embedding_dim,
                n_prototypes,
                use_weight_matrix,
            ...
            model = AnchorBasedCollaborativeFiltering(
                n_objects,
                embedding_dim,
                anchors,
                delta_exc,
