...
class Embedding(FeatureExtractor):
    ...
    def __init__(
        ...
        self.embedding_layer = nn.Embedding(
            self.n_objects, self.embedding_dim, max_norm=self.max_norm
        )
        print(
...
class EmbeddingW(Embedding):
    ...
    def __init__(
        ...
        self.linear_layer = nn.Linear(
            self.embedding_dim, self.out_dimension, bias=self.use_bias
        )

        print(
            f"Built Embeddingw model \n"
...
class AnchorBasedCollaborativeFiltering(FeatureExtractor):
    ...
    def __init__(
        ...
        print(
            f"Built AnchorBasedCollaborativeFiltering module \n"
    ...
    def init_parameters(self):
        ...
        torch.nn.init.normal_(
            self.embedding_ext.embedding_layer.weight, 0, 1
        )  # Overriding previous init
    ...
    def forward(self, o_idxs: torch.Tensor) -> torch.Tensor:
        ...
        assert (
            len(o_idxs.shape) == 2 or len(o_idxs.shape) == 1
        ...
        q_k = (
            o_coeff.reshape(-1, self.n_anchors).sum(axis=0).div(o_coeff.sum())
        )  # [n_anchors]
...
class PrototypeEmbedding(FeatureExtractor):
    ...
    def __init__(
        ...
        use_weight_matrix: bool = False,
        sim_proto_weight: float = 1.0,
        sim_batch_weight: float = 1.0,
        reg_proto_type: str = "soft",
        reg_batch_type: str = "soft",
        ...
        if self.n_prototypes is None:
            self.prototypes = nn.Parameter(
                torch.randn([self.embedding_dim, self.embedding_dim])
            )
            self.n_prototypes = self.embedding_dim
        else:
            self.prototypes = nn.Parameter(
                torch.randn([self.n_prototypes, self.embedding_dim])
            )

        if self.use_weight_matrix:
            self.weight_matrix = nn.Linear(
                self.n_prototypes, self.embedding_dim, bias=False
            )
        ...
        if self.cosine_type == "standard":
            ...
            self.cosine_sim_func = (
                lambda x, y: (1 + nn.CosineSimilarity(dim=-1)(x, y)) / 2
            )
        else:
        ...
        if self.reg_batch_type == "max":
            ...
            raise ValueError(
                f"Regularization Type for Batch {self.reg_batch_func} not yet implemented"
            )
        ...
        if self.reg_proto_type == "max":
            ...
            raise ValueError(
                f"Regularization Type for Proto {self.reg_proto_type} not yet implemented"
            )
        ...
        print(
            f"Built PrototypeEmbedding model \n"
    ...
    def forward(self, o_idxs: torch.Tensor) -> torch.Tensor:
        ...
        sim_mtx = self.cosine_sim_func(
            o_embed.unsqueeze(-2), self.prototypes
        )  # [..., n_prototypes]
...
class ConcatenateFeatureExtractors(FeatureExtractor):
    def __init__(
        self, model_1: FeatureExtractor, model_2: FeatureExtractor, invert: bool = False
    ):
        super().__init__()
        ...
        print(
            f"Built ConcatenateFeatureExtractors model \n"
