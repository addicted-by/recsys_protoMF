...
class Trainer:
    def __init__(
        ...
        self.checkpoint_dir = (
            str(Path(conf.checkpoint_dir) / self.exp) if conf.checkpoint_dir else "."
        )
    ...
    def run(self, trial=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            for u_idxs, i_idxs, labels in track(
            ...
            if curr_value > best_value:
                ...
                torch.save(
                    ...
                    os.path.join(self.checkpoint_dir, f"best_model_{epoch + 1}.pth"),
                )
                patience = 0
            else:
                patience += 1
...
class DistanceTrainer:
    def __init__(
        ...
        if conf.checkpoint_dir:
            if not load_only:
                (Path(conf.checkpoint_dir) / self.exp).mkdir(
    ...
    def run(self, trial=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            for u_idxs, i_idxs, labels in track(
                ...
                user_protos = get_module_by_name(
                    self.model, "module.user_feature_extractor.model_1.prototypes"
                )
                ...
                item_protos = get_module_by_name(
                    self.model, "module.item_feature_extractor.model_1.prototypes"
                )  # self.model['item_feature_extractor.model_1.prototypes']\
                normalized_items_protos = F.normalize(item_protos, p=2, dim=1)
                item_dist = (
                    1 - normalized_items_protos @ normalized_items_protos.T
                )  # item_protos @ item_protos.T
                item_dist = (
                    item_dist.abs().mean()
                )  # (item_dist ** 2).sum() * self.item_alpha
