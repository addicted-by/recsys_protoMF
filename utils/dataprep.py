...
def leave_last_out(data, userid="userid", timeid="timestamp"):
    ...
    remaining = data.drop(
        holdout.index
    )  # store the remaining data - will be our training
    return remaining, holdout


def transform_indices(
    data: pd.DataFrame, users: str, items: str, inplace: bool = False
):
    """
...
def verify_time_split(before, after, target_field="userid", timeid="timestamp"):
    ...
    assert (
        before_ts.reindex(after_ts.index).combine(
            after_ts, lambda x, y: True if x != x else x <= y
        )
    ).all()
