...
from utilities.consts import (CPU_PER_TRIAL, DATA_PATH, GPU_PER_TRIAL, NEG_VAL,
                              NUM_SAMPLES, NUM_WORKERS, OPTIMIZING_METRIC,
                              PROJECT_NAME, SEED_LIST, SINGLE_SEED)
...
def start_training(config, checkpoint_dir=None):
    ...
    trainer = Trainer(
        data_loaders_dict["train_loader"],
        data_loaders_dict["val_loader"],
        data_loaders_dict["test_loader"],
        config,
    )
